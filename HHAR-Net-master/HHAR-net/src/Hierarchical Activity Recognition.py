...
import os
os.chdir(r'C:\Users\GG\Downloads\HHAR-Net-master\HHAR-Net-master')
...
#    
    ...
    parent_labels = ['label:OR_standing','label:SITTING','label:LYING_DOWN',\
                    'label:FIX_running','label:FIX_walking','label:BICYCLING', 'label:PHONE_IN_HAND']
    ...
    best_model_path = r'C:\Users\GG\Downloads\HHAR-Net-master\model.hdf5'
...
#    ###########################################################################
    ...
    print("DNN W/O HIERARCHY RESULTS NOR WEIGHTS FILE------------------------------------------------------------------------------------------->>>>>>>>>>>>>>>>", return_accuracy(clf))
    
    clf_temp.load_weights(best_model_path) #COM O ARQUIVO DE PESOS
    clf_temp.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    
    #return_accuracy(clf_temp)
    print("DNN W/O HIERARCHY RESULTS ------------------------------------------------------------------------------------------->>>>>>>>>>>>>>>>", return_accuracy(clf_temp))
...
#    ###########################################################################
    ...
    print("BASELINE MODELS W/O HIERARCHY --------------------------------ACCURACY-------------------->>>",accuracy)
...
#    ###########################################################################
    ...
    best_model_path = r'C:\Users\GG\Downloads\HHAR-Net-master\model_parent.hdf5'
    checkpointer = ModelCheckpoint(filepath = best_model_path, verbose=1, save_best_only=True)
    ...
    for i in range(len(parent_labels)):
        ...
        best_model_path = r'C:\Users\GG\Downloads\HHAR-Net-master\model_parent.hdf5'.format(i)
        checkpointer = ModelCheckpoint(filepath=best_model_path, verbose=1, save_best_only=True)
        ...
        print("----------------------------------------------- ACCURACY COM HIERARQUIA -----------------------------------------------------------", accuracy)
